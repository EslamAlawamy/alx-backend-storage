...
def call_history(method: Callable) -> Callable:
    """ decorator for Cache class method to track the args """
    @wraps(method)
...
class Cache:
    ...
    def store(self, data: Union[str, bytes,  int,  float]) -> str:
        """ store the data in redis with randomly generated key """
        key = str(uuid4())
        client = self._redis
        client.set(key, data)
        return key
    ...
    def get(self, key: str, fn: Optional[Callable] = None) -> Any:
        """
        gets the key's value from redis and converts
        result byte into the correct data type
        """
        client = self._redis
